...
export const highlightSelected = id => {
  const delActive = Array.from(document.querySelectorAll('.results__link--active'));
  delActive.forEach(el => {
...
export const renderResult = (recipes, page = 1, resPerPage = 10) => {
  // Render results of carent page
  const start = (page - 1) * resPerPage;
  const end = start + resPerPage;

  recipes.slice(start, end).forEach(rendorRecipe);
